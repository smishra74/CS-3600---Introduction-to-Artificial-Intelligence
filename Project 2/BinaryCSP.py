...
def consistent(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if(value== assignment.assignedValues[c.otherVariable(var)] and c.affects(var)):
			return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	if assignment.isComplete():
                return assignment
        current = selectVariableMethod(assignment, csp)
        for value in orderValuesMethod(assignment, csp, current):
                if consistent(assignment, csp, current, value):
                    assignment.assignedValues[current] = value
                    if recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod) != None:
                        return assignment
                    assignment.assignedValues[current] = None
        return None
	#return None
	...
	for var, domain in domains.iteritems():
		if not assignment.isAssigned(var):
			if nextVar == None:
				...
				if len(domain) < len(domains[nextVar]):
					nextVar = var
				elif (not var == nextVar) and (len(domain) == len(domains[nextVar])):
					d1 = 0
					d2 = 0
					for binaryConstraint in csp.binaryConstraints:
                                                if binaryConstraint.affects(nextVar):
							d2 += 1
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	"""YOUR CODE HERE"""
	answer = leastConstrainingValuesHeuristicHelper(assignment,csp,var,values)
	values=[]
	for val in answer:
		values.append(val[0])
...
def leastConstrainingValuesHeuristicHelper(assignment,csp,var,values):
	constrainedValues=[]
	for val in values:
		constraintValues=0
		for c in csp.binaryConstraints:
			if(c.affects(var)):
				variables= c.otherVariable(var)
				domain=list(assignment.varDomains[variables])
				if val in domain:
					constraintValues+=1
		constrainedValues.append((val,constraintValues))
	return sorted(constrainedValues, key=lambda tup: tup[1]) 
...
def forwardChecking(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if(c.affects(var)):
			if(assignment.assignedValues[c.otherVariable(var)]==None):
				constrainedVar.append(c.otherVariable(var))
				variable = c.otherVariable(var)
				length= len(domains[variable])
		...
		if consistent(assignment, csp, var, value):
			...
			if (inferences is not None):   	
				for inference in inferences:
					if inference[1] in assignment.varDomains[inference[0]]:
...
def revise(assignment, csp, var1, var2, constraint):
	...
	for i in assignment.varDomains[var2]:			
		for j in assignment.varDomains[var1]: 		
			if constraint.isSatisfied(j,i):			
				revised = True
				break
			else:
                                continue
	...
	if len(inferences) != 0:
			...
			if len(assignment.varDomains[var])==0: 		
				...
				for variable, value in inferences:
					if value not in assignment.varDomains[variable]:
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints: 		
		if c.affects(var):
			queue.append((c.otherVariable(var), var, c))

	while len(queue)!=0:
		var1,var2,constraint = queue.popleft()

		consistent = revise(assignment, csp, var2, var1, constraint) 	
		...
		if consistent==None: 
			...
			return None
                        ...
                        for c in csp.binaryConstraints:
				if c.affects(var1) and c.otherVariable(var1)!=var2: 
					queue.append((c.otherVariable(var1), var1, c))
		...
		if consistent==None: 
			...
			continue
				...
				if c.affects(var1) and c.otherVariable(var1)!=var2:
					...
					queue.append((var1, c.otherVariable(var1), c))
